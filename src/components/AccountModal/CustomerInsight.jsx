...
export default function CustomerInsight({ draft, update }) {
  const { getContractsForAccount, t, te } = useAccount();
  ...
  const autoBasic = useMemo(() => {
    const items = [
      { key: 'has_contacts', label: t('ins.autoBasic.contacts'), filled: (draft.key_contacts || []).some(c => c.name?.trim()) },
      { key: 'has_dm', label: t('ins.autoBasic.dm'), filled: (draft.key_contacts || []).some(c => c.is_decision_maker) },
      { key: 'has_country', label: t('ins.autoBasic.country'), filled: !!(draft.country && draft.region) },
      { key: 'has_products', label: t('ins.autoBasic.products'), filled: (draft.products || []).length > 0 },
    ];
  ...
  const autoCompetitor = useMemo(() => {
    ...
    const items = [
      { key: 'has_position', label: t('ins.autoCompetitor.position'), filled: !!insight.supplier?.position },
      { key: 'has_competitors', label: t('ins.autoCompetitor.comps'), filled: competitors.length > 0 },
      { key: 'has_substitute', label: t('ins.autoCompetitor.subst'), filled: !!insight.supplier?.substitute_search },
      { key: 'has_strategic', label: t('ins.autoCompetitor.strat'), filled: (insight.supplier?.strategic_products || []).length > 0 },
      { key: 'has_comp_price', label: t('ins.autoCompetitor.price'), filled: competitors.some(c => c.price_level) },
    ];
    const filled = items.filter(i => i.filled).length;
    return { items, filled, total: items.length, pct: Math.round((filled / items.length) * 100) };
  }, [insight]);
  ...
  const autoTrade = useMemo(() => {
    ...
    const items = [
      { key: 'has_contract', label: t('ins.autoTrade.contract'), filled: hasContract },
      { key: 'has_price', label: t('ins.autoTrade.price'), filled: hasPrice },
      { key: 'has_terms', label: t('ins.autoTrade.terms'), filled: hasTerms },
      { key: 'has_moq', label: t('ins.autoTrade.moq'), filled: hasMoq },
      { key: 'has_expiry', label: t('ins.autoTrade.expiry'), filled: hasExpiry },
    ];
    const filled = items.filter(i => i.filled).length;
    return { items, filled, total: items.length, pct: Math.round((filled / items.length) * 100) };
  }, [contracts]);
  ...
  const autoDecision = useMemo(() => {
    const items = [
      { key: 'has_kdm', label: t('ins.autoDecision.kdm'), filled: !!insight.decision?.key_decision_maker },
      { key: 'has_temp', label: t('ins.autoDecision.temp'), filled: !!insight.decision?.relationship_temp },
      { key: 'has_champion', label: t('ins.autoDecision.champion'), filled: insight.decision?.has_champion !== undefined && insight.decision?.has_champion !== '' },
      { key: 'has_influencers', label: t('ins.autoDecision.influencers'), filled: (insight.decision?.influencers || []).length > 0 },
    ];
    const filled = items.filter(i => i.filled).length;
    return { items, filled, total: items.length, pct: Math.round((filled / items.length) * 100) };
  }, [insight]);
        ...
        <div>
          ...
          {stats.items.map(item => (
            <div key={item.key} style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '4px 0', fontSize: 12 }}>
              ...
              <span style={{ color: item.filled ? 'var(--text)' : 'var(--text3)' }}>{te(item.label)}</span>
            </div>
          ))}
        </div>
      )}
            ...
            return (
              <div key={item.key} style={{ display: 'flex', gap: 8, alignItems: 'flex-start', padding: '5px 0', borderBottom: '1px solid rgba(0,0,0,0.04)' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 12, fontWeight: 500, marginBottom: 3, color: complete ? 'var(--text1)' : 'var(--text2)' }}>{te(item.label)}</div>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 4, marginBottom: 16, fontSize: 9, textAlign: 'center' }}>
        {[
          { label: 'Basic', pct: autoBasic.pct },
          { label: 'Market', pct: marketStats.pct },
          { label: 'Competitor', pct: autoCompetitor.pct },
          { label: 'Pain&Needs', pct: painStats.pct },
          { label: 'Trade Terms', pct: autoTrade.pct },
          { label: 'Relationship', pct: autoDecision.pct },
          { label: 'Health', pct: bizHealthStats.pct },
        ].map(s => (
          <div key={s.label}>
          ...
          <div>
            <div className="form-row">
              <div className="form-group">
                <label>Our Supplier Position</label>
            ...
            <div className="form-group" style={{ marginTop: 6 }}>
              <label>Strategic Products</label>
            ...
            <div style={{ marginTop: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
              <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--text2)' }}>Competing Suppliers</label>
              <button className="btn btn-ghost btn-sm" onClick={addCompetitor}>+ 추가</button>
            </div>
            {competitors.length === 0
              ? <div style={{ fontSize: 11, color: 'var(--text3)', textAlign: 'center', padding: 8 }}>No competitors registered</div>
              : competitors.map((c, idx) => (
                <div key={idx} className="contact-card" style={{ position: 'relative' }}>
                  ...
                  <div className="form-row">
                    <div className="form-group"><label>Company Name</label><input type="text" value={c.name || ''} onChange={e => setCompetitor(idx, 'name', e.target.value)} placeholder="Competitor name" /></div>
                    <div className="form-group"><label>Share</label><input type="text" value={c.share || ''} onChange={e => setCompetitor(idx, 'share', e.target.value)} placeholder="e.g., 30%" /></div>
                  </div>
                  <div className="form-row">
                    <div className="form-group"><label>Price Level</label>
                    ...
                    <div className="form-group"><label>Competing Product</label><input type="text" value={c.products || ''} onChange={e => setCompetitor(idx, 'products', e.target.value)} placeholder="Product name" /></div>
                  </div>
                </div>
              ))}
          </div>
      ...
      <div style={SECTION_STYLE}>
        ...
        {expanded.includes('auto_decision') && (
          <div>
            <div className="form-row">
              <div className="form-group"><label>Real Decision Maker</label>
                <input type="text" value={insight.decision?.key_decision_maker || ''} onChange={e => setInsight('decision', 'key_decision_maker', e.target.value)} placeholder="Name / Title" />
              </div>
              <div className="form-group"><label>Relationship Temperature</label>
            ...
            <div className="form-row">
              <div className="form-group"><label>Internal Champion</label>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  {insight.decision?.has_champion && (
                    <input type="text" value={insight.decision?.champion_name || ''} onChange={e => setInsight('decision', 'champion_name', e.target.value)} placeholder="Name/Title" style={{ flex: 1 }} />
                  )}
                </div>
              </div>
            </div>
            <div style={{ marginTop: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
              <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--text2)' }}>Purchase Influencers</label>
              <button className="btn btn-ghost btn-sm" onClick={addInfluencer}>+ 추가</button>
            </div>
            {influencers.length === 0
              ? <div style={{ fontSize: 11, color: 'var(--text3)', textAlign: 'center', padding: 8 }}>No influencers registered</div>
              : influencers.map((inf, idx) => (
                <div key={idx} style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 5 }}>
                  <input type="text" value={inf.name || ''} onChange={e => setInfluencer(idx, 'name', e.target.value)} placeholder="Name" style={{ flex: 1 }} />
                  ...
                  <input type="text" value={inf.dept || ''} onChange={e => setInfluencer(idx, 'dept', e.target.value)} placeholder="Dept" style={{ width: 90 }} />
                  <button className="btn btn-danger btn-sm" onClick={() => removeInfluencer(idx)} style={{ padding: '2px 6px', fontSize: 10 }}>✕</button>
                </div>
              ))}
            <div className="form-group" style={{ marginTop: 8 }}>
              <label>Decision Process Notes</label>
              <textarea value={insight.decision?.process_notes || ''} onChange={e => setInsight('decision', 'process_notes', e.target.value)}
                placeholder="Process to purchase decision, approval steps, lead time, etc." rows={2} style={{ width: '100%', resize: 'vertical', fontSize: 12 }} />
            </div>
          </div>
        )}
      </div>
      ...
      <div style={SECTION_STYLE}>
        ...
        {expanded.includes('biz_health') && (
          <div>
            <div className="form-row">
              <div className="form-group"><label>Revenue Growth Trend</label>
              ...
              <div className="form-group"><label>Budget Trend</label>
            ...
            <div className="form-row">
              <div className="form-group"><label>Contact Change</label>

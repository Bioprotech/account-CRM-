...
| 2026-05-28 | 📊 v3.34 보고서 인사이트 — Loss Reason / Activity ROI / FCST 정확도 / Health Score: 영업본부 CRM 보완 개발 명세서 v1.0 기반 (월간보고 "분석"을 단순 숫자→인사이트로 전환). 신규 헬퍼 lib/reportInsights.js (analyzeLossReasons/analyzeActivityOutcomes/analyzeForecastAccuracy/aggregateHealthScores). ① Loss Reason 보강(P0): GAP 분석 탭에 recoverability(HIGH/MEDIUM/LOW)+competitor_name 필드 추가, 원인 선택 시 자동 박스, 경쟁이탈 시 경쟁사 필수. ⑥ 활동 outcome 태깅(P2): activity_logs.outcome(NO_CHANGE/NEW_LEAD/QUALIFIED/PROPOSAL_SENT/WON/LOST) + 활동추가 폼 드롭다운. Dashboard 신규 카드 "📊 보고서 인사이트"(관리자): Loss Reason 입력률 KPI+미입력 펼침 / Activity ROI(담당자별 WON%) / FCST 정확도(담당자별 6개월) / Health Score 분포(A~D)+위험고객 펼침+자동경고. 월간 보고서 신규 섹션: ■4-4 미달 원인 분석(입력률+카테고리별+회복가능성+경쟁사별+미달고객 상세표), ■4-5 활동→수주 ROI(유형별+담당자별), ■5-2 3개월 예측에 담당자별 FCST 정확도 표시. ⑦ Health Score 간이판(결제 제외 80점: 수주빈도30+응답속도20+클레임율15+FCST정확도15). 보류: ⑤ 마진/공헌이익(ERP/BOM source 결정 필요), ③Stage④경쟁사Win/Loss(Pipeline CRM 영역). 라이브 index-C-s_PkW2.js. |
| 2026-05-28 | 🌐 v3.33 i18n — 한국어/영어 토글 (중국 법인 AM 영문 지원): 사용자 요청 "중국 법인 영업부가 영문 사용". AM 운영 메뉴 영문화 (보고서·설정 제외). (1) i18n 프레임워크 신규 — lib/i18n.js (DICT[lang][key] 사전 + getT(lang) 함수 + translateEnum(value, lang) enum 한→영 매핑). AccountContext에 lang/setLang/t/te 노출. localStorage 영속(crm_lang). Topbar에 [KO|EN] 토글. (2) Phase 1 — 사이드바·Topbar·Dashboard 핵심(계약전환율 KPI 카드·긴급알람·동기화 배너·사용자 환영·활동점수 월선택). (3) Phase 2 — AccountModal 9탭 라벨 전수 영문화. BasicInfo form labels(Company Name/Country/Sales Rep/Contract Status/Products/Key Contacts 등) + 계약전환 가이드 박스. ActivityLog 라벨/드롭다운 옵션/ResolutionModal. PriceContract 헤더·컬럼·버튼. ForecastTrend 헤더. CrossSelling 전체(Summary cards/Current Products/Cross-Selling Opportunities/Form 전체/Item display/Empty state). GapAnalysis 헤더·원인 카드·상세설명·대책·고객 예산/구매 사이클·기회 파이프라인·이번 달 액션 플랜·마지막 업데이트. CustomerInsight + IntelligenceScore 카테고리 라벨·항목·자동감지 라벨·진행바·점수 배지·가중치 표시. (4) Phase 3 — Team Common 페이지+탭·헤더, MyTasks 헤더. (5) Phase 4 — AccountList/OrderReport/Progress 페이지 헤더 + t() context. (6) Enum 한→영 매핑 약 100개 — ISSUE_TYPES/ORDER_ACTIVITY_TYPES/ISSUE_PRIORITIES/CONTRACT_STATUSES/CUSTOMER_CATEGORIES/GAP_CAUSES/RESOLUTION_METHODS/OPPORTUNITY_TYPES/BIZ_HEALTH/SUPPLIER_POSITIONS/RELATIONSHIP_TEMPS/INFLUENCER_ROLES/팀명/Cross-Selling 상태 등. (7) 보고서·Settings는 영문화 제외(사용자 결정). 영문화 안 된 한국어는 기본값(fallback)으로 자동 표시. 라이브 index-DW_q49LC.js. |
| 2026-05-26 | 🎯 v3.32 계약전환율 KPI(Dashboard 상단) + 🚫 거래종료 고객 분류: 사용자 요청 "금년 영업 TASK = 계약전환율. 강력한 메시지 + 거래종료 고객은 활동·전환율·보고서 집계에서 제외". (1) 계약전환율 = 기존 contract_status === '활성' 활용 (별도 필드 추가 X). 활성 판정 가이드(AccountModal 안내박스): 정식계약 / 연간개런티(Diros형) / 정기 FCST 수신(MPS·MRP·월별 발주계획, Mckesson/Cardinal형). (2) 계약(price_contracts) 등록 시 contract_status가 빈 값/'없음'이면 '활성'으로 자동 변경 (override 가능 — AccountContext.saveContractItem에 로직). 계약 있는데 '활성' 아니면 ⚠ 경고. (3) Dashboard 상단 🎯 KPI 카드 신규 (사이드바 메뉴 제거 — 카드 통합) — 본부 전체 % + 담당자별 진행바 + [▼ 미달 N개 보기] 펼침 + 인라인 contract_status 드롭다운 (즉시 변경) + 권한(본부장 전체/담당자 자기 고객만). (4) 신규 customer_category 'inactive'(거래종료) + 적용 범위 = 모든 운영 메뉴 (Dashboard·종합리포트·수주목표관리·진도관리·내업무). 구현: AccountContext openIssues/alarms inactive 자동 제외 + Dashboard myAccounts/Report accounts/OrderReport accounts/Progress accounts/MyTasks accounts 각 view 진입 시 inactive 필터 + scoring.js computeScore inactive 활동 제외. 수주·매출 raw 데이터(orders/sales)는 과거 실적 보존. AccountList는 거래종료 고객도 표시(분류 변경/재활성화 위해). 라이브 index-Bm8h9lka.js 확인. |
| 2026-05-26 | 👥 v3.31 팀 공통 — 활동 + 공통 프로젝트 신규 (고객 단위 외 팀 업무 기록·보고): 사용자 요청 "팀 공통 업무·공통 PJT가 보고서에 누락되던 문제 해결". (1) 신규 Firestore 컬렉션 2종 — team_activities(시점성: 가격정책·정책안내·전시회·교육·법규) + team_projects(지속성+KPI: Smoke 확대 등 전체/세그먼트 PJT). account_id 없음 → 기존 수치/집계 무영향. (2) 신규 사이드바 메뉴 "👥 팀 공통" — 탭 [📣 팀 활동 / 🚀 공통 프로젝트]. 팀 활동: 필터+모달(팀/일자/유형/우선순위/제목/내용/대상/영향고객 typeahead/공유체크). 공통 프로젝트: 카드 그리드(KPI 진행바·다음 마일스톤·최근 update)+편집 모달(KPI 지표명·목표·실적 + 마일스톤 체크 + update 타임라인 + 협업팀 다중체크 + PJT 단위/update 단위 공유 별도). 공유 대상 = 영업3팀+품질/생산/연구개발/경영지원. 작성자/관리자만 삭제. (3) 보고서 자동 통합 — 주간 각 팀 블록 안에 🚀 공통 PJT 진행(주관+협업, 이번 주 update 강조) + 📣 팀 공통 활동(이번 주). 월간 ■3 각 팀 박스에 동일 구조(이달 KPI/update). 주간/월간 팀 블록들 위에 📢 유관부서 공유 사항 callout consolidated — team_activities(공유체크) + team_projects.updates(공유체크)를 한 곳에 합쳐 [활동/PJT] 라벨로 구분 노출. (4) 사이드바 순서 정리: 대시보드→종합리포트→내업무→👥팀공통→고객목록→수주목표관리→진도관리→유형가이드→설정→업데이트. (5) Preview 채널(v331-preview) 사전검토 후 컨펌받아 라이브 배포. 라이브 index-TJ6rvsri.js 확인. |
| 2026-05-26 | 📊 v3.30 주간 분기별 진행 현황 — 사업부(해외/BPU/국내) breakdown 추가: 사용자 요청 "분기별 진행현황을 사업부별로 구분해서". 각 분기 안에 사업부 3개 sub-row + 분기 소계 + 연간 사업부 3개 + 전체 합계 구조로 확장. 헤더 `분기 | 사업부 | 수주(목표/실적/달성률) | 매출(목표/실적/달성률) | 상태`, 분기·상태 셀 rowSpan=4. 데이터: quarterData/salesQuarterData에 byTeam 필드 추가, 기존 헬퍼(getTeamForOrder, getSalesTeamForSale)와 ORDER_TEAM_TO_SALES 매핑 그대로 사용 — ■1·■1-2 사업부 분류와 정확히 일치. 해외기타·해외신규→해외 / 국내기타·국내신규→국내 자동 포함. 분기 소계·전체 합계는 기존 총액과 일치(사업부 미분류 plan은 합계엔 잡히지만 사업부 sub엔 안 잡힘 — 기존 ■1 패턴 유지). 라이브 index-C0Opi2pb.js 확인. |
| 2026-05-25 | 🔮 v3.29 미래 예측 정리 — 차월 파이프라인 제거 + 품목 예측 공식 보정: 사용자 결정 "미래 예측 3개(차월·3개월·품목)가 서로 불일치 → 정리". (1) 차월 수주 파이프라인(■ 5-1) 제거 — 3개월 예측(■ 5-2)의 M+1과 수치 불일치로 혼선 → 차월 금액은 ■ 5-2 M+1 단일 기준으로 통일. 미래 예측 화면 3개→2개(3개월·품목). 자동 Executive Summary의 oppParts "차월 파이프라인 가중" 문구도 제외. monthlyPipeline 계산 로직·return은 유지(미렌더). (2) 품목별 미래예측(■ 9-2) 공식 보정 — 기존 예측=YTD실적+영업활동(사업계획 미반영 → 활동 비면 전 품목 미달=위험). 변경 예측=YTD실적 + 남은 사업계획×50%(기본 planBase=remainingTarget*0.5) + 영업활동. 3 시나리오 모두 planBase 포함(낙관=활동100%/현실=활동가중/비관=FCST50%). 활동 입력 품목은 높게, 미입력은 사업계획 절반이 바닥. (3) 판정 라벨 100% 이분법 — 현실 예측 ≥100% 🟢 초과(목표달성), <100% 🔴 미달 (기존 정상/주의/위험 3단계 → 초과/미달 2단계). 품목 예측 카드 한 곳만 수정, 다른 보고서 수치 무영향. 빌드 캐시 비움→검증(v3.29/초과 라벨/공식 문구/■5-1 제거/■5-2 유지) 후 배포, 라이브 index-Ca7g1YnK.js 확인. |
| 2026-05-25 | 🐛 v3.28 품목별 미래예측 — 크로스셀링 탭 누락 수정: 사용자 지적 "전 품목 위험/주의". 원인=productFutureForecast의 크로스셀링 집계가 account.gap.opportunities만 보고 account.cross_selling[] 탭 누락 (monthlyPipeline은 v3.21에 둘 다 추가됐으나 여기만 빠짐). 수정: ② 크로스셀링에 (a)gap.opportunities + (b)cross_selling[] 둘 다 합산 (선택월 이후+품목매칭+미완료+확률가중). ERBE 등 반영. 한계: 품목별 예측은 영업활동에 품목 기입돼야 매칭(품목 미지정 제외). 라이브 v3.28. |
| 2026-05-25 | 📊 v3.27 주간 분기별 진행 현황 — 매출 추가: 사용자 요청 "분기별 진행현황에 수주만 → 매출도". sectionAData에 salesQuarterData 추가 (목표: team_sales 우선/customerPlans fallback, 실적: sales filterValidSales 분기별). 분기별 진행 현황 표를 2단 헤더(🆕수주 목표/실적/달성률 ‖ 💰매출 목표/실적/달성률 ‖ 상태)로 확장. Q1~Q4 + 연간 합계 수주·매출 나란히. 라이브 v3.27 확인. |
| 2026-05-25 | 🔢 v3.26 주간 ■ 1-4 사업계획 외(신규/기타) 포함 — 전체 금액 일치: 사용자 지적 "수주 19.43억인데 고객별 17.62억, 기타/신규 빠짐". ■ 1-4가 수주목표 보유 고객만 표시해 사업계획 외 1.81억 누락. 수정: 목표 없이 당월 수주 발생한 고객을 plannedAccIds 제외 방식으로 추출 → "🪣 사업계획 외 (신규/기타)" 그룹 추가 (고객명/담당자/당월실적, 목표·달성률 "-", 상태 "계획 외"). 표 구성: 목표 고객 소계 + 사업계획 외 소계 + 전체 합계(grandActual = totalActual + etcActual). 전체 합계 = ■ 1 수주 현황 합계와 정확히 일치. 라이브 v3.26 확인. |
| 2026-05-25 | 📋 v3.25 주간보고 — 고객별 수주목표 진행 현황 신규 (■ 1-4): 사용자 요청 "주간보고에 해당월 수주목표 있는 고객별 실적, meet 여부, 수주활동 정상 이행 점검, 고객별 담당 표시". 주간 보고서 ■ 1-3 담당자별 다음에 ■ 1-4 추가. 이번 달 customer plan targets[monthKey]>0인 고객만 표시. 컬럼: 고객명(클릭→카드)/담당자(account.sales_rep)/당월목표/당월실적(filterValidOrders 당월)/달성률(achieveStyle 색상)/최근활동(activityLogs 마지막+경과일, 30일↑ 빨강)/Open이슈/상태. 상태 자동판정: ✅달성(100%+)/🟡진행중(미달+30일내활동)/🔴점검필요(미달+30일↑또는활동없음). 달성률 낮은순 정렬(점검 필요 상단). 합계행. 라이브 v3.25 확인. |
| 2026-05-25 | 🔗 v3.24 GAP 분석 완전 일원화 — 중복 DEEP GAP ANALYSIS 제거: 사용자 재지적 "GAP 분석 여전히 분산". 확인 결과 Page 5 끝 옛 "DEEP GAP ANALYSIS" 섹션(Report.jsx 7270~7609, 340줄)이 4-3과 중복 잔존. 4개 카드 모두 다른 곳과 중복이라 옮길 필요 없이 중복본만 삭제: Gap 원인 분석(causeRanking)→Page3 ■4-3b / 고객별 심층(topGapCustomers)→Page3 ■4-3 / 기회 파이프라인→Page4 ■5-3(v3.21 모든미래 버전이 더 완전) / AM 활동 품질(amMetrics)→Page5 ■11 담당자 점수. .NET WriteAllLines로 라인 삭제 + 마커 주석, 빌드 검증. GAP이 Page3 ■4-3 한 곳으로 완전 일원화. 내용 손실 0 (Excel용 gapAnalysisData useMemo + 9곳 사용 유지). 지역별 metrics는 사용자 결정으로 대륙별(■9)로 충분, 추가 안 함. |
| 2026-05-25 | 🧹 v3.23 설정창 정리 (누더기 해소): 사용자 지적 "설정창이 거지같이 복잡, 누더기 코드". (1) 고급 도구 4종(고객분류/Account합병/정합성진단/퍼지매칭)을 `<details>` 접이식으로 묶어 평소 숨김 — 첫 화면이 일상 도구만 노출. (2) 죽은 코드 PromesBackfillTool 제거 (185줄, v3.14 imports[] 잔재, 호출처 0건 확인, 화면·기능 영향 없음). 의도적 보류("오류 없음" 원칙): ReconciliationDiagnostic은 고유 기능(사업계획↔실적 매칭 진단) 보유 + 이미 접이식 → 유지 / 섹션 재배치는 거대 인라인 카드 이동 위험 → 현 상태 유지. |
| 2026-05-25 | 🛡 v3.22 긴급 — import 중복 방지 안전장치 (매출 596건 2배 사고 근본 해결): 주간 보고 매출이 실제 15억인데 30억(2배) 표시. 원인=ProMES 매출 재import 시 deleteSalesBySource 실패해도 batchSaveSales 강행 → 298+298=596 중복. 무결성 대시보드가 자동 감지(Import 100.29억↔DB 200.58억). 수정: (1) deleteByQuerySource를 0건 될 때까지 재확인 루프(최대 6라운드)로 강화, 6라운드 후 잔존 시 throw. (2) importOrders/importSales 삭제 실패 시 저장 중단(return) — 중복 원천 차단. (3) countOrdersBySource/countSalesBySource 헬퍼 추가. 정상화: 배포 후 ProMES 매출 재import → 596 완전삭제 → 298 저장. |
| 2026-05-20 | 🔧 v3.21 누락 7건 전면 재작업 (사용자 재지적) + 계정 이관 준비: v3.20에서 5건만 처리·배포한 점 지적 → 7건 모두 단계별 빌드 검증 후 처리. (P1) 자동 Executive Summary 수정 가능 — Page 1 "수동 수정 모드" 체크박스, 4줄 textarea, 월별 Firestore(auto_summary_override_YYYY-MM), 빈칸이면 자동값 사용. (P2) 팀별 활동 분석 상세 항상 표시 — ■3 토글 제거, 신규계약/크로스셀링/미해결이슈 각 건 고객(클릭)·유형·일자·담당·내용 표시, 주요고객컨택·하단 GAP원인·주요이슈TOP 제거. (P3) GAP 4-3+4-3b 한 카드 통합 — 별도 카드 2개를 4-3 안 sub-section(점선 구분)으로. (P4) Page 4 흐름 — ■5 팀별 박스: 다음달계획 textarea + 그 팀 다음달 TASK 리스트 + [+TASK 추가] 인라인(InlineTaskAddForm 신규 컴포넌트) → ■5-1 차월 파이프라인. (P5) ■7 Pipeline CRM 빈 카드 hide — 활성 딜 0건이면 카드 숨김. (P6) 기회 파이프라인 모든 미래 cross_selling — monthlyData.allOpportunities 신규(오늘 이후 모든 cross_selling/gap.opportunities/recovery, 월별 분포 미니표, 제품·확률·예정일 컬럼), ERBE 등 expected_date 무관 즉시 반영. (P7) 담당자 활동점수 월 선택 — Dashboard 📅 드롭다운(최근 6개월), ScoreCardSection/TeamScoreboard yearMonth prop화. 이관 준비: CLAUDE.md 신규(새 Claude 자동 로드 — 작업규칙·절대규칙·핵심파일·현재상태), SESSION_HANDOFF.md 전면 갱신(v3.14→v3.21), ACCOUNT_HANDOVER.md 신규(Claude 구독 변경 이관 가이드). |
...
| 2026-05-31 | 🐛 v3.35 Import 중복 방지 — onSnapshot 경쟁 조건 근본 해결: v3.22에서 Firestore 레벨 중복(delete 실패→2배 저장)은 차단했으나 React state 레벨 중복이 잔존. 원인=batchSaveOrders/batchSaveSales 완료 후 onSnapshot이 전체 state를 교체하는데, 이후 setOrders(prev=>[...prev,...newOrders])가 중복 append. AccountContext.jsx importOrders/importSales에서 ID Set 기반 dedup 적용 — prev에 이미 있는 ID는 toAdd에서 제외. onSnapshot이 먼저 상태 교체했으면 toAdd=[] (no-op), 아직 안 왔으면 optimistic update 그대로 동작 — 어느 타이밍에도 중복 불가. |
| 2026-09-02 | 📊 v3.48 FCST 관리 루프: OrderReport.jsx 📊 FCST 관리 모드 버튼(현재달~+2달 롤링) + 미달 감지(실적<FCST 80% → 빨간 행) + 원인 기록▶(GAP탭 바로가기) + F-up 추가▶(Activity탭 바로가기) + 마지막 F-up 날짜 표시 + 📥 FCST 엑셀 내보내기. AccountContext.jsx openAccountToTab() 신규 — 고객카드를 특정 탭으로 바로 열기. AccountModal.jsx editingAccountInitialTab 연동. |
| 2026-08-06 | 📊 v3.47 GAP 분석 7종 개선: (①) OrderReport.jsx effectiveFcsts — FCST가 확정수주 전환 달은 GAP에서 제외, FCST 컬럼 추가. (②) BasicInfo.jsx rep_locked 플래그 + Dashboard.jsx handleSync에서 잠금 고객 제외, 잠금 배지 UI. (③) MyTasks.jsx repTeam 다수결 방식으로 버그 수정. (④) Dashboard.jsx weeklyStatus 현황판 카드 추가. (⑤) Report.jsx 4-3/4-4 분리 재편. (⑥) GapAnalysis.jsx gap_cause_history 배열 누적 + 이번달저장 버튼 + 이력 뷰. (⑦) Report.jsx ■3-1 전체 F-up 활동 이력 뷰 추가. |
| 2026-07-22 | 📋 v3.46 액션플랜 복수화+강제화+고객분석+대시보드알림: ActivityLog.jsx next_actions 배열 도입(기존 next_action/due_date 하위호환). 로그 추가 시 최소 1개 액션(내용+기한) 필수. Report.jsx 차주계획·전주이행점검 배열 대응. Dashboard.jsx 이번주액션마감 카드 추가(기한초과/오늘이내/이번주 구분). BasicInfo.jsx 고객분석 섹션 추가(7개선택필드+관심내역+특이사항). constants.js SCORE_CATEGORIES에 customer_analysis 카테고리 추가(weight 0.09), 기존 가중치 비례조정. |
| 2026-07-10 | 👥 v3.45 공동 담당자(co_reps) 기능: BasicInfo.jsx에 공동 담당자 체크박스 추가. AccountContext.jsx visibleAccounts 필터에 co_reps 포함. AccountModal.jsx canEdit에 co_reps 포함. 주 담당자(sales_rep) 유지 — 리포트/집계 무영향. |
| 2026-06-29 | 📥 v3.44 고객카드 Excel 완전 내보내기: AccountModal.jsx 활동로그 시트 컬럼 10개 추가(priority/예상금액/제품군/대상품목/수주세부유형/회복계획3개/타부서공유/outcome). 매출이력 시트 신규(sale_date/제품군/매출금액/담당자/합계). Insight 시트 신규(공급자포지션/경쟁사/의사결정자/인플루언서/사업건전성). 기본정보 시트에 고객분류·전략등급·Context메모 추가. |
| 2026-06-24 | 🏷 v3.43 경쟁사 가격 Excel 가져오기: CompetitorView.jsx에 📤 가져오기 버튼 추가. 내보낸 Excel 편집 후 재업로드로 일괄 수정 가능. ID 있는 행=기존 수정, ID 없는 행=신규 추가. 가져오기 전 신규/수정 건수 확인창. 내보내기에 ID 컬럼 + 안내 메모 추가. |
| 2026-06-24 | 🏷 v3.42 경쟁사 가격 자사 비교 + 비고: CompetitorView.jsx에 ourModel/ourDealerPrice/ourHospitalPrice/remarks 필드 추가. 테이블 경쟁사 vs 자사 2열 비교 구조(대리점가·병원납품가 각 경쟁/자사 sub-열). 자동 가격비교 뱃지(▼% 녹색=자사 저렴, ▲% 적색=자사 고가). 모달에 자사가격 전용 섹션 추가. Excel 비고 컬럼 포함. |
| 2026-06-24 | 🏷 v3.41 경쟁사 가격 관리 신규: CompetitorView.jsx 신규 (사이드바 🏷 경쟁사 가격). Firestore competitors 컬렉션 신규. firebase.js subscribeCompetitors/saveCompetitor/deleteCompetitor 추가. AccountContext.jsx 구독·콜백·context value 추가. 필터 4종(국가/카테고리/경쟁사/통화). 입력/수정 모달(고객사 연결 포함). 삭제 관리자 전용. Excel 내보내기. firestore.rules 명시적 권한 추가. |
| 2026-06-21 | 🗂 v3.40 사이드바 메뉴 통합 정리: 수주목표관리·품목별 수주현황을 진도관리 내 탭으로 통합. Progress.jsx에 mainTab 상태 추가, OrderReport·ProductReport import 후 탭 전환으로 렌더링. Sidebar.jsx에서 orderReport·productReport 항목 제거, 메뉴 9개로 정리 (대시보드→리포트→내업무→고객목록→진도관리→팀공통→유형가이드→업데이트내역→설정 순). App.jsx에서 불필요 import·case 제거. |
| 2026-06-21 | 📦 v3.39 품목별 수주현황 신규 뷰: ProductReport.jsx 신규 (사이드바 📦 품목별 수주현황). 제품군 탭(전체+16품목) 클릭→고객별 실적. KPI 카드(연간목표/YTD/달성률/전년동기). 전체 보기시 제품군별 요약+드릴다운. 고객별 상세(해외/BPU/국내, 담당자, 비중 바, 정렬). 고객명 클릭→AccountModal. Excel 내보내기(H1/H2/연간 구조). filterValidOrders() 적용. App.jsx import+case 추가, Sidebar.jsx 탭 추가, i18n.js 한/영 키 추가. |
| 2026-06-21 | ✏️ v3.38 주간보고 비고란 + FCST 수정 + Plate P 품목 추가: (1) Report.jsx — ■1 수주현황 / ■1-2 매출현황 / ■1-5 다음달 예상 테이블에 비고 ✏️ 컬럼 추가. 팀별·합계 행에 이슈 메모 입력 칸 신규 (orders_notes / sales_notes / notes 키, 동일 Firestore 저장 구조). (2) ForecastTrend.jsx — FCST 상세 항목 테이블 [수정] 버튼 추가, 클릭 시 기존 값을 입력폼에 자동 로드, 동일 id로 upsert 저장, 취소 버튼 지원. (3) constants.js — PRODUCTS 배열에 'Plate P' 추가 (Cross-selling 품목 선택에 표시). |
| 2026-06-09 | 🎯 v3.37 월간보고 개선 + 수주현황 분석 대시보드: 지적사항 대응 "내용은 많은데 무슨 얘기인지 모르겠다". (1) Report.jsx — Page 1 최상단에 결론 헤드라인 배너 추가: 상태 아이콘(🟢🟡🔴) 큰 글자 + ■0 핵심 메시지 3줄(미입력 시 자동 생성 요약) + 수주MTD/수주YTD/매출YTD 빠른 KPI 3종 — 회의 시 첫 화면에서 결론 즉시 파악 가능. (2) Report.jsx — Page 2 섹션 접기/펼치기: ■2-3 담당자별 / ■8 품목별 / ■9 대륙별 기본값 접힘(▶ 펼치기 버튼), 회의 시 필요한 섹션만 펼쳐 발표. (3) Dashboard.jsx — OrderAnalysisCard 신규: 엑셀 수주현황_분석 형태 테이블(해외계획고객/해외신규기타/국내계획고객/국내기타신규/BPU 섹션), 연간계획/YTD계획/YTD실적/달성률/전년YTD/계획대비/전년대비 7컬럼, 섹션 클릭→고객별 펼침, 전체 담당자 공통 열람 가능. |
| 2026-05-31 | 📅 v3.36 주간보고 ■ 1-5 — 다음 달 수주/매출 예상 (월말·월초 시점): 사용자 요청 '월이 넘어가는 시점 주간보고에 다음 달 수주/매출 예상 포함'. 표시조건: 이번 주 시작일 ≥ 15일 OR 이미 입력 데이터 존재. ■1-4 고객별 수주목표 카드 다음에 ■1-5 카드 신규 — 팀별(해외/BPU/국내) 수주예상+매출예상 input + 합계 행. 저장 키 next_month_forecast_YYYY-MM (다음달 기준, 어느 주차든 동일 값 유지). 600ms debounce Firestore 자동 저장. 보라색 좌측 border 시각 구분. Report.jsx state nextMonthForecast + nmForecastKey + useEffect + updateNextMonthForecast 핸들러 + showNextMonthForecast 조건 추가. |
